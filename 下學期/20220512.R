...
COVID_19=read.csv("japan_owl_world.csv")
COVID_19_vac=read.csv("japan_owl_world_vac.csv")
View(COVID_19)
View(COVID_19_vac)
...
names(COVID_19_part)<-c("date","total_vaccination","daily_vaccination","daily_vaccination_avg","die","population")
names(COVID_19_vac_part)<-c("date","people_vaccinated_per_hundred","people_fully_vaccinated_per_hundred","total_boosters_per_hundred")
...
View(COVID_19_part)
COVID_19_part_2022<-subset(COVID_19_part,date<="2022-05-16 CST"&date>="2022-01-01 CST")

##劃圖累計
dev.new()
ggplot()+
  geom_line(data=COVID_19_part_2022,#data.frame的資料
            aes(x=date,y=daily_vaccination,colour = "daily_vaccination"),size=1)+#折線
  geom_line(data=COVID_19_part_2022,#data.frame的資料
            aes(x=date,y=daily_vaccination_avg,colour = "daily_vaccination_avg"),size=1)+
  theme_bw()+
  annotate("text" #標籤
           ...
           ,label=paste(COVID_19_part_2022$date[which.min(COVID_19_part_2022$daily_vaccination_avg)],"Lo" 
  ...
  annotate("text" #標籤
           ...
           ,label=paste(COVID_19_part_2022$date[which.max(COVID_19_part_2022$daily_vaccination_avg)]
                        ...
                        ,"第一劑",COVID_19_part_2022$people_vaccinated_per_hundred[which.max(COVID_19_part_2022$daily_vaccination_avg)],"%"
                        ,"第二劑",COVID_19_part_2022$people_fully_vaccinated_per_hundred[which.max(COVID_19_part_2022$daily_vaccination_avg)],"%"
                        ,"第三劑",COVID_19_part_2022$total_boosters_per_hundred[which.max(COVID_19_part_2022$daily_vaccination_avg)],"%"
                        ,"當時累計確診占總人口",round(COVID_19_part_2022$total_vaccination[which.max(COVID_19_part_2022$daily_vaccination_avg)]/
                         COVID_19_part_2022$population[which.max(COVID_19_part_2022$daily_vaccination_avg)],4)*100,"%"),colour="black",size=3
                        ,vjust=-1.5)+
  ...
  ggtitle(paste("Japan 2022 COVDI-19 ","daily_vaccination_avg","Low to High increase"
                ,round(COVID_19_part_2022$daily_vaccination_avg[which.max(COVID_19_part_2022$daily_vaccination_avg)]/
                 COVID_19_part_2022$daily_vaccination_avg[which.min(COVID_19_part_2022$daily_vaccination_avg)]-1,0)
                ,"倍","duration",interval(start = COVID_19_part_2022$date[which.min(COVID_19_part_2022$daily_vaccination_avg)]
                ,end = COVID_19_part_2022$date[which.max(COVID_19_part_2022$daily_vaccination_avg)]) / duration(num = 1, units = "days") 
                ,"days","目前累計確診占總人口",round(tail(COVID_19_part_2022$total_vaccination,1)/tail(COVID_19_part_2022$population,1),4)*100,"%"))+
  theme(plot.title = element_text(face="italic",colour = "red",hjust = 0.5,size = 15))#標題
