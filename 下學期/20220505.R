...
str(COVID_19_part)#將格式改成日期格式及數字格式
...
View(COVID_19_part)
COVID_19_part_2022<-subset(COVID_19_part,date<="2022-05-03 CST"&date>="2022-01-01 CST")
...
ggplot()+
  ...
  ggtitle(paste("South Korea 2022 COVDI-19 ","daily_vaccination_avg","Low to High increase"
                ...
                ,"%","duration",interval(start = COVID_19_part_2022$date[which.min(COVID_19_part_2022$daily_vaccination_avg)]
                ,end = COVID_19_part_2022$date[which.max(COVID_19_part_2022$daily_vaccination_avg)]) / duration(num = 1, units = "days") 
                ,"days","累計確診占總人口",round(head(COVID_19_part_2022$total_vaccination,1)/head(COVID_19_part_2022$population,1),2)*100,"%"))+
